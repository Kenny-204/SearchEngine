...
namespace SearchEngine.Query.Services
{
  ...
  public class QueryParser : IQueryParser
  {
    ...
    {
      _stemmingService =
        stemmingService ?? throw new ArgumentNullException(nameof(stemmingService));
    ...
    public QueryRepresentation Parse(string query)
    {
      ...
      if (string.IsNullOrWhiteSpace(query))
      {
        throw new ArgumentException(
          "Query cannot be null, empty, or whitespace only",
          nameof(query)
        );
      }
      ...
      // Step 4: Tokenization
      var tokens = Regex
        .Split(cleanedQuery, _configuration.TokenizationPattern)
        .Where(token => !string.IsNullOrEmpty(token))
        .Select(token => token.Trim())
        .Where(token => !string.IsNullOrEmpty(token))
        .Where(token =>
          token.Length >= _configuration.MinimumWordLength
          && token.Length <= _configuration.MaximumWordLength
        )
        .ToList();
